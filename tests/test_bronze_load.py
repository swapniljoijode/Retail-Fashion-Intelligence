...
class TestBronzeSchema:
    ...
    def test_source_file_column_present(self, bconn):
        for tbl in _TABLE_GLOBS:
            cols = [
                row[0] for row in bconn.execute(f"DESCRIBE bronze.{tbl}").fetchall()
            ]
    ...
    def test_load_timestamp_column_present(self, bconn):
        for tbl in _TABLE_GLOBS:
            ...
            assert (
                "_load_timestamp" in cols
            ), f"_load_timestamp missing from bronze.{tbl}"
...
class TestBronzeRowCounts:
    ...
    def test_ingestion_log_has_one_entry_per_table(self, bconn):
        count = bconn.execute("SELECT COUNT(*) FROM bronze.ingestion_log").fetchone()[0]
...
class TestBronzeAppendOnly:
    def test_second_load_doubles_row_count(self, appendable_db):
        ...
        conn = duckdb.connect(db_path)
        for tbl in _TABLE_GLOBS:
            count_after = conn.execute(f"SELECT COUNT(*) FROM bronze.{tbl}").fetchone()[
                0
            ]
            assert (
                count_after == counts_before[tbl] * 2
